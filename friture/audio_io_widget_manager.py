...
class AudioIOWidgetManager(QtWidgets.QWidget):

    def __init__(self, parent, logger=PrintLogger()):
        super(AudioIOWidgetManager, self).__init__(parent)
    ...
    def set_io_widgets(self, io_widgets, widget_index):
        ...
        self.parent().connect_io_widget(self.audio_widget)

        tool_tip_text = "Select the Audio I/O Widget"
        io_widget_names = [widget.objectName() for widget in io_widgets]
        self.control_bar.add_widgets(io_widget_names, widget_index, tool_tip_text)
